...
// var device = Cap.findDevice('192.168.98.200'); /// that's the source IP you find in github repo example @ https://github.com/mscdex/cap#examples
var device = Cap.findDevice('192.168.98.236'); /// that's the source IP i want to ping from
var filter = 'arp';
...
var buffer = Buffer.from([
    ...
    0xb4, 0xb0, 0x24, 0xd5, 0x03, 0x8c,                 // 6    = Source MAC (that's what i change as MAC Addresss of Paker Windows Host on TP Link USB Wifi Network Adpater)
    // B4-B0-24-D5-03-8C
    // 0xb4, 0xb0, 0x24, 0xd5, 0x03, 0x8c,
    // 0x08, 0x00, 0x 27, 0x2e, 0xee, 0x51,
    ...
    0xb4, 0xb0, 0x24, 0xd5, 0x03, 0x8c,                 // 22/8   = Sender Hardware Addr (MAC) (that's what i change as MAC Addresss of PAker Windows Host on TP Link USB Wifi Network Adpater)
    // 0xc0, 0xa8, 0x01, 0xc8,                             // 28/14  = Sender Protocol address (ipv4)
    // above : 192.168.1.200  ==  0xc0, 0xa8, 0x01, 0xc8, (given as example by https://github.com/mscdex/cap#examples )
    // below : 192.168.98.236  ==  0xc0, 0xa8, 0x62, 0xec,
    0xc0, 0xa8, 0x62, 0xec,                             // 28/14  = Sender Protocol address (ipv4)
    ...
    // above : 192.168.1.201  ==  0xc0, 0xa8, 0x01, 0xc9, (given as example by https://github.com/mscdex/cap#examples )
    // below : 192.168.98.156  ==  0xc0, 0xa8, 0x62, 0x9c, (that's the IP Address i want to ping)
    // below : 192.168.98.157  ==  0xc0, 0xa8, 0x62, 0x9d, (that's the IP Address i want to ping)
    0xc0, 0xa8, 0x62, 0x9d
...
try {
    ...
    /**
     * JBL notes : 
     * - why not do that in rust ?
     * - indeed see https://github.com/rust-pcap/pcap/pull/22
     */
    console.log(` >>>> JBL : just before sending packet`)
    c.send(buffer, buffer.length);
    console.log(` >>>> JBL : just after sending packet`)
