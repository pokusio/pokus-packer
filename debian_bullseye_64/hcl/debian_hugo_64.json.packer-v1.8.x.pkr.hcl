...
source "virtualbox-iso" "debian_hugo_64" {
  boot_command     = ["<esc><wait>install <wait>", "preseed/url=http://${var.PackerHostIPAddrWifi}:{{ .HTTPPort }}/${var.preseed_path} <wait>", "debian-installer=en_US.UTF-8 <wait>", "auto=true <wait>", "priority=critical  <wait>", "interface=enp0s3 <wait>", "netcfg/get_hostname=dev.pokusio.io <wait>", "netcfg/get_domain=pokusup.com <wait>", "locale=en_US.UTF-8 <wait>", "kbd-chooser/method=us <wait>", "keyboard-configuration/xkb-keymap=fr <wait>", "fb=false <wait>", "debconf/frontend=noninteractive <wait>", "console-setup/ask_detect=false <wait>", "console-keymaps-at/keymap=fr <wait>", "grub-installer/bootdev=default ", "PACKER_AUTHORIZED_KEY=%3Cno+value%3E<enter>", "${local.boot_command_env_addon}", " <wait><enter><wait>"]
  boot_wait        = "5s"
  cpus             = "${var.cpus}"
  ...
  # ssh_host         = "${var.created_vm_ipaddr}"
  ssh_host         = "192.168.164.167"
